...
This is a naive model for several reasons:
<ol>
...
However, when accounting for the hourly/non-hourly status, the models show
...
The general equation for the model is: $$\hat Y = b_{0} + b_{1}X_{i}$$
...
This model is naive for several reasons:
<ol>
...
Testing the linearity assumptions of this model can start with the summary of
the linear model:
...
From the results of the regression, the model becomes:

$$weekly\ earnings = 611.7809 + 6.6874(age) - 179.994(sex) - 
493.1572(full/part\ status) + 479.0939(hourly/nonhourly\ status)$$

There does not appear to be any obvious colinearity, as 
...
A hypothetical observation might be a person who is 40 years old, male, works
...
```
The predicted weekly earnings for this observation would be $`r result`.
